...
// GET /api/roles/permissions & /api/roles/permissions/catalog - List all available system permissions
router.get(['/permissions', '/permissions/catalog'], verifyToken, async (req, res) => {
  try {
...
// GET /api/roles/audit & /api/roles/audit/permissions - Permission Audit View: Retrieve user-role-permission mapping (Requires ROLE_MANAGE)
router.get(['/audit', '/audit/permissions'], verifyToken, requirePermission('ROLE_MANAGE'), async (req, res) => {
  try {
